...
Storage.prototype.emit = function(event) {
  if (event == 'request') {
    this._addDefaultRequestListeners(this);
  }
...
Storage.prototype._addDefaultRequestListeners = function(self) {
  validators.request.forEach(function(fn) { self.once('request', fn) });
};
